...
function FHapi(cfg, logr) {
  ...
  api.bootstrap = function(cb) {
    api.db({'act': 'connectionString'}, function(err, res) {
      api.events.emit('db.connection.done', {url: res});
      return cb(err, res);
    });
  };
  ...
  api.events.on('db.connection.done', function(data) {
    logr.info('Done with retreving mongodb connection string:' + util.inspect(data));
  });
