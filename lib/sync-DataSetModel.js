...
var self = {
  ...
  getDatasetClient: function (dataset_id, params, cb) {
    ...
    self.getDataset(dataset_id, function (err, dataset) {
      if (err) return err;
      console.log('getClientHashBefore');
      self.getClientHash(query_params, meta_data, function(err, clientHash) {
        console.log('getClientHash');
        if (err) return cb(err);
  ...
  getClientHash: function (query_params, meta_data, cb) {
    syncUtil.generateHash(query_params, function(err, queryParamsHash) {
      if (err) return cb && cb(err);

      syncUtil.generateHash(meta_data, function(err, metaDataHash) {
        if (err) return cb && cb(err);

        return cb && cb(null, queryParamsHash + '-' + metaDataHash);
      });
    });
  },
  ...
  doSyncList: function (dataset, datasetClient, cb) {
    ...
    self.doListHandler(dataset.id, datasetClient.queryParams, function (err, records) {
      ...
      async.forEachSeries(records, function addToRecordList(record, itemCallback) {
        syncUtil.generateHash(record, function(err, hash) {
          hashes.push(hash);
      ...
      }, function recordListComplete(err) {
        syncUtil.generateHash(hashes, function(err, globalHash) {
