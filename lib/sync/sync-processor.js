...
function getCollisionCount(datasetId, metaData, cb) {
  dataHandlers.countCollisions(datasetId, metaData, cb);
}
...
function syncWithBackend(payload, tries, callback) {
  ...
  async.waterfall([
    ...
    function countCollisions(recordsWithHash, cb) {
      getCollisionCount(datasetId, metaData, function(err, collisionCount) {
        return cb(err, recordsWithHash, collisionCount);
      });
    },
    function saveRecords(recordsWithHash, collisionCount, cb) {
      // Hash the hashes of all records along with the collision count so clients won't ignore collisions and never update
      var toHash = _.pluck(recordsWithHash, 'hash').concat([collisionCount]);
