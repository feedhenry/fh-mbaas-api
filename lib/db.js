...
function net_db(params, callback) {
  ...
  if (params.act === 'import'){
    // Only import operations can't be streamed - formdata doesn't make it through alas
  ...
  if (params.files || params.act === 'import'){
    var form = req.form();
    ...
    return; // Callback will be dealt with by request's callback arg
  }

  // All requests other than ones with files attached get streamed.
