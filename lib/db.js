...
function mongoConnectionStringOS2(cb){
  ...
  apiEvents.emit('db.connection.success', {url: connectionString});
  apiEvents.emit('db.connection.done', {url: connectionString});
...
function mongoConnectionStringOS3(cb){
  ...
  mbaasClient.app.databaseConnectionString({
    ...
    if (err) {
      apiEvents.emit('db.connection.error', {error: err});
      apiEvents.emit('db.connection.done', {error: err});
      return cb(err);
    }
    process.env.FH_MONGODB_CONN_URL = resp.url;
    apiEvents.emit('db.connection.success', {url: resp.url});
    apiEvents.emit('db.connection.done', {url: resp.url});
...
function getMongoConnectionUrl(cb){
  ...
  if(process.env.FH_MONGODB_CONN_URL){
    ...
    apiEvents.emit('db.connection.success', {url: process.env.FH_MONGODB_CONN_URL});
    apiEvents.emit('db.connection.done', {url: process.env.FH_MONGODB_CONN_URL});
    return cb(undefined, process.env.FH_MONGODB_CONN_URL);
    ...
    apiEvents.emit('db.connection.error', {error: 'no way to determine mongo connection string'});
    apiEvents.emit('db.connection.done', {error: 'no way to determine mongo connection string'});
    return cb();
  }
}
