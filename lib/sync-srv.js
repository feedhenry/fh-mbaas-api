      ...
      acknowledgeUpdates(dataset_id, params, function(err) {
        ...
        if( params.pending && params.pending.length > 0) {
          ...
          if( datasetClient.data.hash ) {
            ...
            //syncUtil.doLog(dataset_id, 'verbose', 'doClientSync - No pending - Hash (Client :: Cloud) = ' + params.dataset_hash + ' :: ' + datasetClient.data.hash, params);
            ...
            if( datasetClient.data.hash === params.dataset_hash) {
              ...
              syncUtil.doLog(dataset_id, 'info', 'doClientSync - No pending - Hashes NO NOT match (Client :: Cloud) = ' + params.dataset_hash + ' :: ' + datasetClient.data.hash + ' - return full dataset', params);
              // TODO - Partial Sync
              var res = datasetClient.data;
              returnUpdates(dataset_id, params, res, callback);
            }
          } else {
            syncUtil.doLog(dataset_id, 'verbose', 'No pending records. No cloud data set - invoking list on back end system', params);
            DataSetModel.syncDatasetClientObj(datasetClient, function(err, res) {
              if( err ) callback(err);
              returnUpdates(dataset_id, params, res, callback);
            });
...
function processPending(dataset_id, dataset, params, cb) {
  ...
  var itemCallback = function(err, update) {
    syncUtil.doLog(dataset_id, 'verbose', 'itemCallback :: err=' + err + " :: storedPendingUpdate = " + util.inspect(update), params);
  }
  ...
  async.forEachSeries(pending, function(pendingObj, itemCallback) {
      ...
      if( "create" === action ) {
        syncUtil.doLog(dataset_id, 'info', 'CREATE Start', params);
        ...
        syncUtil.doLog(dataset_id, 'info', 'UPDATE Start', params);
        DataSetModel.doReadHandler(dataset_id, uid, function(err, data) {
          ...
          syncUtil.doLog(dataset_id, 'info', ' READ for UPDATE Success', params);
          ...
          syncUtil.doLog(dataset_id, 'info', 'UPDATE Hash Check ' + uid + ' (client :: dataStore) = ' + preHash + ' :: ' + dataHash, params);
        ...
        syncUtil.doLog(dataset_id, 'info', 'DELETE Start', params);
        DataSetModel.doReadHandler(dataset_id, uid, function(err, data) {
          if( err ) {
          ...
          syncUtil.doLog(dataset_id, 'info', ' READ for DELETE Success', params);
          syncUtil.doLog(dataset_id, 'silly', ' READ for DELETE Data : \n' + util.inspect(data), params);

          var preHash = syncUtil.generateHash(pre);
          var dataHash = syncUtil.generateHash(data);

          syncUtil.doLog(dataset_id, 'info', 'DELETE Hash Check ' + uid + ' (client :: dataStore) = ' + preHash + ' :: ' + dataHash, params);
...
function returnUpdates(dataset_id, params, resIn, cb) {
  ...
  $fh.db({
    ...
    if( res.list.length > 0 ) {
      syncUtil.doLog(dataset_id, 'info', 'returnUpdates :: ' + util.inspect(updates.hashes), params);
    }
...
function acknowledgeUpdates(dataset_id, params, cb) {
  ...
  var itemCallback = function(err, update) {
    syncUtil.doLog(dataset_id, 'verbose', 'acknowledgeUpdates :: err=' + err + ' :: update=' + util.inspect(update), params);
  }

  if( updates && updates.length > 0) {
    syncUtil.doLog(dataset_id, 'info', 'acknowledgeUpdates :: ' + util.inspect(updates), params);
      ...
      for( i in serverRecs ) {
        ...
        if( clientRecs[serverRecUid] ) {
          if( clientRecs[serverRecUid] !== serverRecHash ) {
            syncUtil.doLog(dataset_id, 'info', 'Updating client record ' + serverRecUid + ' client hash=' + clientRecs[serverRecUid], params);
            updates[serverRecUid] = serverRec;
          }
        } else {
          syncUtil.doLog(dataset_id, 'info', 'Creating client record ' + serverRecUid, params);
