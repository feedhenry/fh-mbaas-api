        ...
        if( params.pending && params.pending.length > 0) {
          ...
          if( datasetClient.dataHash ) {
            ...
            if( datasetClient.dataHash === params.dataset_hash) {
              ...
              syncUtil.doLog(dataset_id, 'info', 'doClientSync - No pending - Hashes NO NOT match (Client :: Cloud) = ' + params.dataset_hash + ' :: ' + datasetClient.dataHash + ' - return full dataset', params);
              // TODO - Partial Sync
              DataSetModel.getDatasetClientRecords(datasetClient.id, function(err, data) {
                if(err) return callback(err);
                var res = {hash: datasetClient.dataHash, records: data};
                return returnUpdates(dataset_id, params, res, callback);
              });
            }
          } else {
...
function doSyncRecords(dataset_id, params, callback) {
  ...
  DataSetModel.getDatasetClient(dataset_id, params.query_params, params.meta_data, function(err, datasetClient) {
    ...
    if( datasetClient.dataHash) {
      ...
      DataSetModel.getDatasetClientRecords(datasetClient.id, function(err, serverRecs) {
