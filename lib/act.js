...
var act = function() {
  ...
  function doAppCall(callurl, actParams, reqParams, cb) {
    ...
    if( actParams.path ) {
      callurl += actParams.path;
    } else {
      callurl += "/cloud/" + actParams.endpoint;
    }

    var urlObj = url.parse(callurl);
    ...
    request(reqOpts , function (error, response, body) {
      ...
      return cb(null, body, response);  // maintaining backward compatibility with earlier flawed api
    });
